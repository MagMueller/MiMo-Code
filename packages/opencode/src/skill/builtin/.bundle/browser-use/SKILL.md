---
name: browser-use
description: "Default for browser interaction: navigation, form filling, automation, scraping, screenshots, and site/app verification via Browser Use CLI. Use webfetch or websearch for public information that needs no browser interaction."
...
Browser Use CLI is the default for browser interaction in MiMoCode. Follow an explicit user request for a different browser tool. Run browser commands through the shell tool; no MCP server or separate LLM API key is needed for local Chrome.

Check `command -v browser-use` first. If the CLI is missing, install it with:
